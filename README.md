...
- Manual stitch mode for gaps too large for the auto tolerance: click "Start Manual Stitch", then click anywhere on one line, then anywhere on the line you want it joined to (or the same line again, to close it into a loop) — no need to hit an exact endpoint, the nearest matching pair of ends is resolved and joined automatically, which matters when two loose ends are only meters apart and visually overlap even at max zoom. Manual joins persist through re-stitching and can be removed individually. Clicks always resolve to the specific line actually clicked, not just whichever line happens to be nearest, so a busy junction with several close, near-parallel lines won't snap to the wrong one
