...
The sidebar is organized into four tabs — **Data**, **Stitch**, **Measure**, **Export** — so only one group of controls shows at a time, and can be collapsed entirely (✕ button in its header) to give the map more room, reopened with the ☰ button that appears in the top-left corner.

Once a Path Trace is finished, the **Export** tab's "Segment & export" panel lets you mark which of the placed waypoints are actual stations (the rest stay in as shape detail within whichever segment they fall in) and name each station. Each station gets a permanent pin marker with an always-visible name label on the map — no hover needed — so stations stand out clearly against the track and any waypoints. From there you can export a `track.geojson` + `loop_lengths.csv` pair matching the `track_pipeline.py` contract.
...
- Build a station/segment table from a finished trace, with permanent labeled pin markers for stations, and export it as `track.geojson` + `loop_lengths.csv`
